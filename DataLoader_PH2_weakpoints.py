...
class PH2_weekpoints(torch.utils.data.Dataset):
    ...
    def __init__(self, transform, number_of_points,train,  data_path=data_path):
        'Initialization'
        self.train = train
        ...
        if train:
            self.image_paths = self.image_paths[0:180]
            self.label_paths = self.label_paths[0:180]
        else:
            self.image_paths = self.image_paths[180:]
            self.label_paths = self.label_paths[180:]
    def __len__(self):
